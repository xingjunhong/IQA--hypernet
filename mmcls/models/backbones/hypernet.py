...
class HyperNet(BaseBackbone):
    ...
    """
    def __init__(self, lda_out_channels, hyper_in_channels, target_in_size, target_fc1_size, target_fc2_size, target_fc3_size, target_fc4_size, feature_size1,feature_size2):
        ...
        self.fc1w_conv = nn.Conv2d(self.hyperInChn, int(self.target_in_size * self.f1 / feature_size1 *feature_size2/16), 3,  padding=(1, 1))#** 2
        self.fc1b_fc = nn.Linear(self.hyperInChn, self.f1)

        self.fc2w_conv = nn.Conv2d(self.hyperInChn, int(self.f1 * self.f2 / feature_size1 *feature_size2/16), 3, padding=(1, 1))
        self.fc2b_fc = nn.Linear(self.hyperInChn, self.f2)
        #
        self.fc3w_conv = nn.Conv2d(self.hyperInChn, int(self.f2 * self.f3 / feature_size1 *feature_size2/16), 3, padding=(1, 1))
        self.fc3b_fc = nn.Linear(self.hyperInChn, self.f3)

        self.fc4w_conv = nn.Conv2d(self.hyperInChn, int(self.f3 * self.f4 / feature_size1 *feature_size2/16), 3, padding=(1, 1))
        self.fc4b_fc = nn.Linear(self.hyperInChn, self.f4)

        self.fc5w_conv = nn.Conv2d(self.hyperInChn, int(self.f4**2/ feature_size1 *feature_size2/16), 3, padding=(1, 1))
        self.fc5b_fc = nn.Linear(self.hyperInChn, 1)

        # self.conv5 = nn.Conv2d(24,1,1)
        # self.fc5 = nn.Linear(1,1)
            # nn.ReLU(inplace=True),
            # nn.Conv2d(12,1,1,1,1),
            # nn.ReLU(inplace=True),
    ...
    def forward(self, img):
        feature_size1 = self.feature_size1
        feature_size2 = self.feature_size2
        ...
        hyper_in_feat = self.conv1(res_out['hyper_in_feat']).view(-1, self.hyperInChn, feature_size1, feature_size2)
        ...
        target_fc5w = self.fc5w_conv(hyper_in_feat).view(-1, self.f4, self.f4, 1, 1)
        ...
        l1 = nn.Sequential(
            TargetFC(out['target_fc1w'], out['target_fc1b']),
            nn.Sigmoid(),
        )
        l2 = nn.Sequential(
            TargetFC(out['target_fc2w'], out['target_fc2b']),
            nn.Sigmoid(),
        )
        #
        l3 = nn.Sequential(
            TargetFC(out['target_fc3w'], out['target_fc3b']),
            nn.Sigmoid(),
        )
        #
        l4 = nn.Sequential(
            TargetFC(out['target_fc4w'], out['target_fc4b']),
            nn.Sigmoid(),
            TargetFC(out['target_fc5w'], out['target_fc5b']),
            # nn.Linear(24,1)
        )
...
class TargetFC(nn.Module):
    ...
    def forward(self, input_):
        ...
        input_re = input_.view(-1, input_.shape[0] * input_.shape[1], input_.shape[2], input_.shape[3])#224*224尺寸 input_.shape[0] * input_.shape[1]
        ...
        out = F.conv2d(input=input_re, weight=weight_re, bias=None, groups=self.weight.shape[0])
        out1 = out.view(input_.shape[0], self.weight.shape[1], input_.shape[2], input_.shape[3])
        return out1
...
class ResNetBackbone(nn.Module):
    ...
    def __init__(self, lda_out_channels, in_chn, block, layers, num_classes=1000):
        ...
        self.lda1_fc = nn.Linear(832, lda_out_channels)#224*224尺寸  16 * 64 832

        self.lda2_pool = nn.Sequential(
            nn.Conv2d(512, 32, kernel_size=1, stride=1, padding=0, bias=False),
            nn.AvgPool2d(7, stride=7),
        )
        self.lda2_fc = nn.Linear(384, lda_out_channels)#224*224尺寸  32 * 16 384

        self.lda3_pool = nn.Sequential(
            nn.Conv2d(1024, 64, kernel_size=1, stride=1, padding=0, bias=False),
            nn.AvgPool2d(7, stride=7),
        )
        self.lda3_fc = nn.Linear(192, lda_out_channels)#224*224尺寸  64 * 4  192

        self.lda4_pool = nn.AvgPool2d(3, stride=3)#224*224尺寸 7,stride=7
        self.lda4_fc = nn.Linear(8192, in_chn - lda_out_channels * 3)#224*224 2048 8192
