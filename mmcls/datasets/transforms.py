...
from .compose import MultiImage
...
class PhotoMetricDistortion:
    ...
    def __call__(self, results):
        ...
        results['img'] = np.clip(img, 0.0, 255.0)
        return results
...
class Mosaic:
    ...
    def _mosaic_transform(self, results):
        ...
        Returns:
            dict: Updated result dict.
        """
        if not results.get('mix_results', False):
            return results
        mosaic_labels = []
        mosaic_bboxes = []
        ...
        for i, loc in enumerate(loc_strs):
            if loc == 'top_left':
                ...
                results_patch = copy.deepcopy(results['mix_results'][i])
...
@PIPELINES.register_module()
class RandomMosaic():
    def __init__(self, num_images=3,  image_size=640, mosaic_ratio=1,
                 mosaic_border=(-320, -320),img_fill_val=(0,0,0)):
    ...
    def load_mosaic(self, results_list):
        # loads images in a 4-mosaic
        labels4, bboxes4, segments4 = [],[],[]
        s = self.image_size
        ...
        for i, results in enumerate(results_list):
            ...
            if bboxes.size:
                bboxes[:, [0, 2]] += padw
                bboxes[:, [1, 3]] += padh
                #labels = self.xywhn2xyxy(labels, w, h, padw, padh)  # normalized xywh to pixel xyxy format
            bboxes4.append(bboxes)
            labels4.append(labels)
        # Concat/clip labels
        bboxes4 = np.concatenate(bboxes4, 0)
        labels4 = np.concatenate(labels4, 0)
        for x in (bboxes4, *segments4):
            np.clip(x, 0, 2 * s, out=x)  # clip when using random_perspective()

        return img4, bboxes4.astype(np.float32), labels4.astype(np.float32)
    ...
    def __call__(self, results):
        if not results.get('mix_results', False):
            return results
        if self.num_images > len(results['mix_results']):
            print_log('num_images={} 小于 len(mix_results)={} '
                      .format(self.num_images, len(results['mix_results']), level=logging.WARNING))
            self.num_images = len(results['mix_results'])

        results_expand = results['mix_results']
        results_list = [result_expand for result_expand in results_expand[:self.num_images]]
        img, gt_labels, gt_bboxes = results['img'], results['gt_labels'], results['gt_bboxes']
        results_list.append(results)
        if random.random() <= self.mosaic_ratio:
            img, gt_bboxes, gt_labels = self.load_mosaic(results_list)
...
@PIPELINES.register_module()
class ChipeakFilter(object):
    def __init__(self, obj_min_ratio=None, obj_min_size=None):
        if obj_min_ratio is not None:
            assert isinstance(obj_min_ratio,float) and  0<=obj_min_ratio<=1
        if obj_min_size is not None:
            assert isinstance(obj_min_size,(int,float))
        self.obj_min_ratio = obj_min_ratio
        self.obj_min_size = obj_min_size
    ...
    def __call__(self,results):
        ...
        gt_bboxes_area = (gt_bboxes[:,2] - gt_bboxes[:,0]) * (gt_bboxes[:,3] - gt_bboxes[:,1])
        pos_idx = None
        if self.obj_min_ratio:
            img_area = results['img_shape'][0] * results['img_shape'][1]
            gt_bboxes_ratio = gt_bboxes_area/img_area
            pos_idx = gt_bboxes_ratio > self.obj_min_ratio
        if self.obj_min_size:
            pos_idx = gt_bboxes_area > self.obj_min_size

        results['gt_labels'] = gt_labels[pos_idx] if pos_idx is not None else gt_labels
        results['gt_bboxes'] = gt_bboxes[pos_idx] if pos_idx is not None else gt_bboxes
        return results
...
class RandomPerspective(object):
    def __init__(self, degrees=0.0, translate=.1, scale=.5, shear=0.0, perspective=0.0, border=(-320, -320)):
    ...
    def __call__(self, results):
        ...
        if n:
            # warp points
            pts = np.ones((n * 4, 3))
            pts[:, :2] =gt_bboxes[:, [0, 1, 2, 3, 0, 3, 2, 1]].reshape(n * 4, 2)  # x1y1, x2y2, x1y2, x2y1
            pts = pts @ M.T  # transform
            if self.perspective:
                pts = (pts[:, :2] / pts[:, 2:3]).reshape(n, 8)  # rescale
            else:  # affine
            ...
            x = pts[:, [0, 2, 4, 6]]
            y = pts[:, [1, 3, 5, 7]]
            pts = np.concatenate((x.min(1), y.min(1), x.max(1), y.max(1))).reshape(4, n).T
            ...
            # clip boxes
            pts[:, [0, 2]] = pts[:, [0, 2]].clip(0, width)
            pts[:, [1, 3]] = pts[:, [1, 3]].clip(0, height)

            # filter candidates
            i = self.box_candidates(box1=gt_bboxes[:, 0:4].T * s, box2=pts.T)
...
class ChipeakRandomHhsv:
    ...
    def __call__(self, results):
        ...
        cv2.cvtColor(img_hsv, cv2.COLOR_HSV2BGR, dst=img).astype(np.float32)  # no return needed
        results['img'] = img
        return results