...
class Pad(object):
    ...
    def _pad_img(self, results):
        ...
        # if r'data/reflective/train/1/yes_reflective_1188.jpg ' == results['filename']:
        #     print(results['filename'], results['img_shape'])
        for key in results.get('img_fields', ['img']):
            if self.size is not None:
                ...
                #     img = mmcv.imresize(img,(128,64))

                if h>=w:

                    # if (0<img.shape[0]<=self.size[0]) and (0<img.shape[1]<=self.size[1]):
                    img = mmcv.imrescale(img,(self.size[0],self.size[1]))
                    # cv2.imwrite('demo/ '+ results['img_info']['filename'],img)
                    # cv2.imwrite('demo/ ' +results['img_info']['filename'].split('/')[2], img)
                        # print(img.shape)
                    # else:
                    #     img= mmcv.imresize(img,(128,64))
                else:
                    ...
                    # img = mmcv.imresize(img,(96,32))

                # H,W = self.size
                # padded_img = cv2.copyMakeBorder(results[key],)
            ...
            # a = 'demo/ ' + results['img_info']['filename'].split('/')[2]
        # cv2.imwrite('demo/pading/ ' + results['img_info']['filename'].split('/')[2], padded_img)
        results['pad_shape'] = padded_img.shape

        # cv2.imwrite(r'output/picture/pad18.jpg',padded_img)
